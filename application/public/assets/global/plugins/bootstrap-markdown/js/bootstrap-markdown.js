      ...
      [{
        ...
        data: [{
          ...
          callback: function(e){
            ...
            if (selected.length == 0) {
              // Give extra word
              chunk = e.__localize('Текст')
            } else {
              chunk = selected.text
            }

            link = prompt(e.__localize('Введите IMG URL'),'http://')

            if (link != null && link != '' && link != 'http://' && link.substr(0,4) == 'http') {
              var sanitizedLink = $('<div>'+link+'</div>').text()
              
              // transform selection and set the cursor into chunked text
              e.replaceSelection('<img src="'+sanitizedLink+'" alt="'+chunk+'"> '+e.__localize('</img>')+'')
              cursor = selected.start+2

              // Set the next tab
              e.setNextTab(e.__localize('</img>'))
